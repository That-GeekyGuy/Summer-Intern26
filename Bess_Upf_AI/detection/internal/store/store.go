...
type Event struct {
	...
	RuleName              string
	Severity              string
	EventType             string   // "reactive" (default) or "predictive"
	ForecastHorizon       string   // e.g. "1h"; empty for reactive events
	PredictedCrossingTime *int64   // Unix seconds when trend crosses capacity; nil for reactive
	Confidence            *float64 // R² [0,1]; nil for reactive events
	ThresholdConfig       string   // JSON {"capacity":N,"metric":"..."}; empty for reactive
}
...
// columnMigrations adds predictive-event columns to pre-existing databases.
...
// eventType can be "", "reactive", or "predictive".
func (s *Store) List(ctx context.Context, since time.Time, metric, severity, eventType string) ([]QueryResult, error) {
	...
	for rows.Next() {
		var r QueryResult
		var ts, ca int64
		var fh, threshCfg sql.NullString
