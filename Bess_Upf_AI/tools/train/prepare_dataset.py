...
def _load_feedback(db_path: str) -> pd.DataFrame:
    """Load operator feedback from the audit database."""
    if not db_path or not os.path.exists(db_path):
        return pd.DataFrame()
    try:
        import sqlite3
        con = sqlite3.connect(db_path)
        df = pd.read_sql_query("SELECT metric_name, timestamp, is_true_positive FROM anomaly_feedback", con)
        con.close()
        return df
    except Exception as e:
        log.warning("Failed to load feedback from %s: %s", db_path, e)
        return pd.DataFrame()
...
def run(source: str = "local", db_path: str = None):
    ...
    # ── 6b. Apply Operator Feedback ───────────────────────────────────────────
    if db_path:
        feedback_df = _load_feedback(db_path)
        if not feedback_df.empty:
            log.info("Applying %d operator feedback records", len(feedback_df))
            for _, row in feedback_df.iterrows():
                # We do a loose matching: operator dismissed an anomaly at roughly this time
                # In a real system, you'd match the exact window or episode.
                # Here we just set the binary label at the exact timestamp down to 15s bucket.
                fb_ts = pd.to_datetime(row["timestamp"], unit="s", utc=True)
                
                # Find nearest timestamp within 15 seconds
                time_diffs = (df["timestamp"] - fb_ts).abs()
                nearest_idx = time_diffs.idxmin()
                if time_diffs[nearest_idx].total_seconds() <= 15:
                    if row["is_true_positive"] == 0:
                        df.at[nearest_idx, "uoi_binary"] = 0
                        log.info("Feedback applied: Overrode anomaly to NORMAL at %s", df.at[nearest_idx, "timestamp"])
                    elif row["is_true_positive"] == 1:
                        df.at[nearest_idx, "uoi_binary"] = 1
                        log.info("Feedback applied: Confirmed anomaly at %s", df.at[nearest_idx, "timestamp"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source", choices=["local", "minio", "synthetic"], default="local")
    parser.add_argument("--db-path", default=None, help="Path to audit database containing anomaly_feedback")
    args = parser.parse_args()
    run(source=args.source, db_path=args.db_path)
